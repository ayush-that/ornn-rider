...
const MAX_WHEEL_AV = 1.15 // cap back-wheel angular speed (rad/step)
const WHEEL_ACCEL = 0.12 // spin-up per throttle step
const BRAKE_DECEL = 0.09 // brake/reverse approach rate
const REVERSE_TARGET_AV = -0.28 // slow reverse spin under brake
const WHEELIE_TORQUE = 0.007 // engine-reaction nose-lift per step
const MAX_WHEELIE_AV = 0.10 // clamp on wheelie rotation build-up
...
export function createTerrainBodies(engine: Engine, terrain: Terrain, startX: number): TerrainCollider {
  ...
  function update(bikeX: number): void {
    ...
    // Teleport (reset/retry far away): new window doesn't overlap the active
    // range — rebuild directly instead of sweeping add/remove across the gap,
    // which on long tracks touches thousands of bodies in one frame.
    if (hiIdx > loIdx && (xs[hiIdx - 1] < lo || xs[loIdx] > hi)) {
      for (let i = loIdx; i < hiIdx; i++) Composite.remove(engine.world, bodies[i])
      let a = 0
      let b = n
      while (a < b) {
        const mid = (a + b) >> 1
